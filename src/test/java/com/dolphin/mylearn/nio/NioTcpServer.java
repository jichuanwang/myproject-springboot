            ...
            while(true) { // 轮询
                ...
                if(nKeys>0) {
                    ...
                    while(it.hasNext()) {
                        ...
                        if(key.isAcceptable()) {
                            PrintUtil.print("Server: SelectionKey is acceptable.");
                            handler.handleAccept(key);
                        } else if(key.isReadable()) {
                            PrintUtil.print("Server: SelectionKey is readable.");
                            handler.handleRead(key);
                        } else if(key.isWritable()) {
                            PrintUtil.print("Server: SelectionKey is writable.");
...
class ServerHandler implements Handler {
    ...
    public void handleRead(SelectionKey key) throws IOException {
        ...
        while(true) {
            ...
            if(readBytes>0) {
                byteBuffer.flip();
                PrintUtil.print("Server: readBytes = " + readBytes);
                PrintUtil.print("Server: data = " + new String(byteBuffer.array(), 0, readBytes));
                byteBuffer.clear();
                StringBuilder sendString = new StringBuilder();
                sendString.append("HTTP/1.1 200 OK\r\n");
                sendString.append("Content-Type:text/html;charset=utf8\r\n\r\n");
                sendString.append("hello world333333333333333333333333333333333333");
                byteBuffer.put(sendString.toString().getBytes());
                byteBuffer.flip();
